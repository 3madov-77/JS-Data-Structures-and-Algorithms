...
const addNumbers = (num, arr, times, callback) => {
  // Solution code here...
    for(let i =0; i < times; i++){
      ...
      // maybe in difrent univrese.. 🙄
    //   Array.prototype.forEach = function(callBack){
    //       for(let i = 0; i < times; i++){
    //           return callBack(this[i]);
    //       }
    //   }
    //   arr.forEach(item => item = callback(arr, num))
    };
...
const createList = availableItems => {
  // Solution code here...
  let finalList = [];
  availableItems.forEach((item) => item.available == true? finalList.push(item.name) : '');
    return finalList;

    // 4 me on the future .. use this line! its much esiear :)
    // return availableItems.filter(item => item.available === true);
};
...
const fizzbuzz = (arr) => {
  // Solution code here...
  let outputArray = [];
  arr.forEach(item => {item % 3 == 0 && item % 5 == 0? outputArray.push(`Fizz Buzz`) : item % 3 == 0? outputArray.push(`Fizz`) : item % 5 == 0? outputArray.push(`Buzz`) : outputArray.push(item)})
  return outputArray;
};

...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(item => {
    result.push(item.split(' ').slice(2, 99).join(' '));
  })
  return result;
};
...
const totalSumCSV = (str) => {
  ...
  // Solution code here...
  var x = str.split(',')
  x.forEach(item => total += parseInt(item));
  return total;
};
...
const removeVowels = (str) => {
  // Solution code here...
    ...
    var x = str.split('')
    x.forEach(item => {
      if(item != 'e'&& item != 'o'&& item != 'i'&& item != 'u'&& item != 'a'){
      total += item
      }
    });
    return total;
...
const extractVowels = (str) => {
  ...
  vowelsArr.sort()
  vowelsArr.forEach(item => vowels += item)
  return [total, vowels];
};
